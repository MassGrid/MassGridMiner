...
uint64_t blkmk_init_generation3(blktemplate_t * const tmpl, const void * const script, const size_t scriptsz, bool * const inout_newcb) {
	...
	*inout_newcb = false;
	...
	int16_t sigops_count;
	int64_t gentx_weight;
	if(tmpl->masternode_payments_started && tmpl->masternode_address!= NULL && tmpl->masternode_script!=NULL){
			memcpy(&data[off],
			...
			my_htole64(&data[off], tmpl->cbvalue-tmpl->mnvalue);
			off += 8;
			data[off++] = scriptsz;
			if (scriptsz) {
			...
			//masternode payee
			my_htole64(&data[off], tmpl->mnvalue);
			off += 8;
			data[off++] = scriptsz;
			if (scriptsz) {
				memcpy(&data[off], tmpl->masternode_script, scriptsz);
				off += scriptsz;
			}
			...
			const int16_t sigops_counted = blkmk_count_sigops(script, scriptsz, tmpl->_bip141_sigops)+ blkmk_count_sigops(tmpl->masternode_script, scriptsz, tmpl->_bip141_sigops);
			gentx_weight = blkmk_calc_gentx_weight(data, off);
			sigops_count = sigops_counted;
