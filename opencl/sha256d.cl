 ...
 */
__constant const WORD h[] = { 0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19 };
...
void processChunk(SHA256_CTX *ctx) {
...
__kernel void scanHash_post(__global char* input,__global uint* goodNonce, const ulong target)
{
	...
	hash_t32 output;
	unsigned char hashdata[64];
	for(int i=0;i<64;++i)
		hashdata[i]=input[64*gid+i];
	size_t inLen = 64;
	SHA256_CTX ctx;
	SHA256Initialize(&ctx);
	SHA256Update(&ctx, (BYTE *)hashdata, inLen);
	...
	if (result) {
		...
		goodNonce[goodNonce[FOUND]++]=gid;
	}
	barrier(CLK_GLOBAL_MEM_FENCE);
}
__kernel void  hashTest(__global char* in, __global char* outp)
{
	unsigned char input[64],output[32];
	for(int i=0;i<64;++i)
		input[i]=in[i];
	size_t inLen = 64;
	SHA256_CTX ctx;
	SHA256Initialize(&ctx);
	SHA256Update(&ctx, (BYTE *)input, inLen);
	SHA256Finalize(&ctx, (BYTE *)output);
	for(int i=0;i<32;++i)
		outp[i]=output[i];
}
